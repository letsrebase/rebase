...
const CODERABBIT_LOOP_LINES = [
  ...
  "  summary=$(gh api --paginate \"repos/" + args.org + "/" + args.repo + "/issues/<n>/comments?per_page=100\" \\",
  ...
  "done",
  "echo \"$summary\" | grep -q \"\\\"coveredCommitId\\\":\\\"$sha\\\"\" || echo \"summary still not on $sha: unverified, gate not met\"",
  ...
  "```",
  "",
  "The summary is edited in place on every round: the last line above prints whether",
  "it actually names this sha yet, and a missing `crid` only means the round raised",
  "nothing once that line confirms it does, never on \"summary still not on $sha\" alone,",
  "which means the review is unverified and the gate is not met, not that it passed.",
  "Each finding is either **fixed**, in a commit that names it, or",
  ...
  "marked ready; on no status, or a summary saying the reviews are paused (it pauses",
  "itself after five reviewed commits on one PR, to spare the hourly allowance), run",
...
const ADVERSARIAL_LOOP_LINES = [
  ...
  ...GREPTILE_LOOP_LINES.slice(2, 32),
  "before going on. Ten minutes with no run on the head sha: comment `@greptileai`",
  "once, which re-triggers it, and wait again. What each finding gets, once a run",
  "is there, is in (d) below, not here: the one mistake to avoid is replying on",
  "Greptile's own thread before CodeRabbit's adversarial verdict has landed.",
  "",
  ...
  "nothing new and its check run reads `success`, or, on the rarer run that leaves",
  "only a review with no check run, its score reads full marks; CodeRabbit's status",
  "on it reads",
