...
const GREPTILE_BLOCK = args.greptile
  ? [
      ...
      "repos/" + args.org + "/" + args.repo + "/pulls/<n>/comments/<id>/replies -F body=@file`, never `-f` with a",
      "backtick in the body, bash reads it as a command substitution). An answered",
      "finding still counts against the score until the thread is resolved (`gh api",
