...
const GREPTILE_BLOCK = args.greptile
  ? [
      ...
      "echo \"$run\" | cut -f2-",
      "[ \"$(echo \"$run\" | cut -f1)\" = success ] && echo \"check run passes on $sha\" \\",
      "  || echo \"check run does not pass on $sha yet\"",
      "```",
      "",
      ...
      "the new sha, read again: the check run's own `conclusion` is what the merge gate",
      "itself reads, so it is what decides here too, never a score read from free text",
      "that a push could leave stale. The loop ends when the check run on the sha that",
      "will merge reads `success`. Ten minutes with no run on the head sha: comment",
