...
- [ ] **Step 2:** On the preview: `REBASE_ANTHROPIC_API_KEY` in the preview hub's `.env` (Ivan holds the key: ask him for it on the card and wait), restart `api`, `rebase cards-refresh` in the container until it prints «0 schede scritte», open `/hub/team`, describe a project, read the team, regenerate, file a request with a test address, read it in «Richieste team».
